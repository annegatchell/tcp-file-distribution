...
int connect_to_other_client(char *server_ip){
    ...
    //Populate the server inso struct
    if((status = getaddrinfo(server_ip, C_TO_C_PORTNUM, &hints, &servinfo)) != 0){
        fprintf(stderr, "getaddrinfo: %s\n", gai_strerror(status));
    }   
    //Loop through all the results and bind to the first we can
    for(p = servinfo; p != NULL; p = p->ai_next){
        if((client_to_clientfd = socket(p->ai_family, p->ai_socktype, 
            p->ai_protocol)) == -1){
            perror("Client could not set up socket\n");
            continue;
        }

        if ((status = connect(client_to_clientfd, p->ai_addr, p->ai_addrlen)) == -1) {
            close(client_to_clientfd);
